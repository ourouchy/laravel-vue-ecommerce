    ...
    <div class="mt-1 flex rounded-md shadow-sm">
        ...
        <template v-else>
            <input :type="type"
                   ...
                   :value="props.modelValue"
                   @input="emit('update:modelValue', $event.target.value)"
                   :class="inputClasses" :placeholder="label" step="0.01"
            />

        </template>
        <span v-if="append"
            class="inline-flex items-center p-3 rounded-r-md border border-l-8 border-gray-300 bg-gray-50 text-gray-500 text-sm">
            {{append}}
        </span>
    </div>
</div>
